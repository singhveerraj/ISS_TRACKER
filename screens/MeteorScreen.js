...
getMeteors = () => {
    axios
    .get("https://api.nasa.gov/neo/rest/v1/feed?api_key=jO0jcgUVFDCztema81IsJe0UU1dubZDCjjE293U5")
    .then(response => {
...
renderItem = ({item}) => {
 ...
 <View>
  <ImageBackground source = {bg_img} style = {styles.backgroundImage}>
    ...
    <View>
    <Text style={[styles.cardTitle, { marginTop: 380, marginLeft: 50 }]}>{item.name}</Text>
    <Text style={[styles.cardText, { marginTop: 5, marginLeft: 50 }]}>Closest to Earth - {item.close_approach_data[0].close_approach_date_full}</Text>
